...
init python:
    ...
    while (k < 7):
        l = 0
        while (l < 3):
            town.occupy (29 + k+buffer, 13 + l+buffer, wall)
            l+= 1
        k += 1
    ...
    while (m < 7):
        n = 0
        while (n < 3):
            town.occupy (18 + m+buffer, 14 + n+buffer, wall)
            n+= 1
        m += 1
    ...
    cave = MapDenizen (23+buffer, 16+buffer, "house door.png", 49, 49, cave)
    ...
    lancer = MapDenizen (30+buffer, 15+buffer, "lancer.png", 72, 70, shop)
    town.occupy (30+buffer, 15+buffer, lancer)
